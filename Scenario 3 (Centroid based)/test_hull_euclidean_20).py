...
#        im2, contourszz, hierarchy = cv2.findContours(target_mask, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
        ...
        if len(contours) != 0 and np.max(cnt_area) > 100:
            ...
            #         np.cos(angles)]).T
                ...
                rval[3] = np.dot([x1, y1], r)
                                ...
                                #Centroid
                cx1=int((rval[0][0]+rval[2][0])/2.0)
                cx2=int((rval[0][1]+rval[2][1])/2.0)
                cy1=int((rval[1][0]+rval[3][0])/2.0)
                cy2=int((rval[1][1]+rval[3][1])/2.0)
