...
def siamese_track(state, im, centeroid, mask_enable=False, refine_enable=False, device='cpu', debug=False):
    ...
    if mask_enable:
        ...
        if len(contours) != 0 and np.max(cnt_area) > 100:
            ...
            polygon = contour.reshape(-1, 2)
            kotak = cv2.boxPoints(cv2.minAreaRect(polygon))  # Rotated Rectangle

           # if len(edges) >= 2:
            if len(hulk) <= 4:
                prbox = kotak
                cx1=int((kotak[0][0]+kotak[2][0])/2.0)
                cx2=int((kotak[0][1]+kotak[2][1])/2.0)
                cy1=int((kotak[1][0]+kotak[3][0])/2.0)
                cy2=int((kotak[1][1]+kotak[3][1])/2.0)
                ...
                centeroid = np.array(centeroid).reshape(-1,1)
                tengah = np.array(tengah).reshape(-1,1)


                D = dist.cdist(centeroid, tengah)


                if abs(D[0,0]-D[1,1]) <= 20 and abs(D[0,1]-D[1,0]) <= 20:
                    prbox = np.array([ao,bo,co,do])
                else:
                    cx1=int((kotak[0][0]+kotak[2][0])/2.0)
                    cx2=int((kotak[0][1]+kotak[2][1])/2.0)
...
def track_vot(model, video, hp=None, mask_enable=False, refine_enable=False, device='cpu'):
    ...
    for f, image_file in enumerate(image_files):
        ...
        if f == start_frame:  # init
            ...
            centeroid = np.array([cx, cy])

            state = siamese_init(im, target_pos, target_sz, model, hp, device)  # init tracker
            location = cxy_wh_2_rect(state['target_pos'], state['target_sz'])
            regions.append(1 if 'VOT' in args.dataset else gt[f])
        elif f > start_frame:  # tracking
            state, centeroid = siamese_track(state, im, centeroid, mask_enable, refine_enable, device, args.debug)  # track
            if mask_enable:
