...
def siamese_track1(state, im, mask_enable=False, refine_enable=False, device='cpu', debug=False):
    p = state['p']
    net = state['net']
...
def siamese_track2(state, im, centeroid, mask_enable=False, refine_enable=False, device='cpu', debug=False):
    p = state['p']
    net = state['net']
    avg_chans = state['avg_chans']
    window = state['window']
    ...
    if mask_enable:
        ...
        back_box = [-sub_box[0] * s, -sub_box[1] * s, state['im_w'] * s, state['im_h'] * s]
        mask_in_img = crop_back(mask, back_box, (state['im_w'], state['im_h']))
        # gam_in_img = crop_back(mask, back_box, (state['im_w'], state['im_h']))

        target_mask = (mask_in_img > p.seg_thr).astype(np.uint8)
        # gray = cv2.cvtColor(target_mask, cv2.COLOR_BGR2GRAY)
        # blurred = cv2.GaussianBlur(gray, (5, 5), 0)
        # _, thresh = cv2.threshold(blurred, 20, 255, cv2.THRESH_BINARY)
        # thresh = cv2.Canny(thresh, 20, 100)

        if cv2.__version__[-5] == '4':
            contours, _ = cv2.findContours(target_mask, cv2.RETR_LIST, cv2.CHAIN_APPROX_NONE )
        else:
            _, contours, _ = cv2.findContours(target_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE) #Lebih lama ini dari pada bawah
            # _, contours, _ = cv2.findContours(target_mask, cv2.RETR_LIST, cv2.CHAIN_APPROX_NONE) #
            # _, contours, _ = cv2.findContours(target_mask, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE) #banyak kotak kecil
            
            # _, contours, _ = cv2.findContours(target_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)  #ada kotak kecil
            # _, contours, _ = cv2.findContours(target_mask, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE) #ada kotak kecil
            # _, contours, _ = cv2.findContours(target_mask, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE) #lumayan banyak kotak kecil
        cnt_area = [cv2.contourArea(cnt) for cnt in contours]
        if len(contours) != 0 and np.max(cnt_area) > 100:
            contour = contours  # use max area polygon
            # polygon = contour.reshape(-1, 2)
            # pbox = np.array(cv2.boundingRect(polygon))  # Min Max Rectangle
            # print(polygon)

            #Kasih if kurang dari 2 untuk hull point
            
            hull = np.zeros(len(contours))
            # print(convex_hull_image(target_mask))
            ...
            #print(hull)
            # hull = np.array(hull).reshape(len(hull))
            ...
            hull_points = hull.flatten()
            # print(hull_points)
            # print(hull)

            hulk=[]
            hulk = np.zeros((int(len(hull_points)/2),2))
            j=0
            k=0
            for i in range(0,len(hull_points),2):
                hulk[j,0]=hull_points[i]
                j+=1
            for jj in range(1,len(hull_points),2):
                hulk[k,1]=hull_points[jj]
                k+=1

            so, ao, bo, co, do, z1o, z2o  = mep(hulk)

            # print(so, ao, bo, co, do, z1o, z2o )
            prbox = np.array([ao,bo,co,do])

            #Centroid
            cx1=int((prbox[0][0]+prbox[2][0])/2.0)
            cx2=int((prbox[0][1]+prbox[2][1])/2.0)
            cy1=int((prbox[1][0]+prbox[3][0])/2.0)
            cy2=int((prbox[1][1]+prbox[3][1])/2.0)

            ccx=int((cx1+cy1)/2.0)
            ccy=int((cx2+cy2)/2.0)
            tengah = np.array([ccx,ccy])


            # cv2.circle(im, (ccx,ccy), 4, (0, 255, 0), -1)
...
def siamese_track3(state, im, mask_enable=False, refine_enable=False, device='cpu', debug=False):
    p = state['p']
    net = state['net']
    avg_chans = state['avg_chans']
    window = state['window']
    ...
    if mask_enable:
        ...
        if cv2.__version__[-5] == '4':
            ...
            # _, contours, _ = cv2.findContours(target_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE) #ada cuma satu
            _, contours, _ = cv2.findContours(target_mask, cv2.RETR_LIST, cv2.CHAIN_APPROX_NONE) #gak ada kotak kecil #susah dibedain sama RETR_LIST satunya, sama atasnya juga
            # _, contours, _ = cv2.findContours(target_mask, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE) #banyak kotak kecil
            
            # _, contours, _ = cv2.findContours(target_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)  #banyak kotak kecil
            # _, contours, _ = cv2.findContours(target_mask, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE) #gk ada kotak dan gak gagal
            # _, contours, _ = cv2.findContours(target_mask, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE) #lumayan banyak kotak kecil
        ...
        if len(contours) != 0 and np.max(cnt_area) > 100:
            ...
            edges = np.zeros((int(len(hull_points)/2)-1, 2))
            edges = hulk[1:] - hulk[:-1]
        ...
        #         np.cos(angles)]).T
            ...
            rot_points = np.dot(rotations, hulk.T)
